...
final baseUrlProvider = Provider<String>((ref) => EnvHandler.instance.envProperties.baseUrl);

final serviceDioProvider = Provider<Dio>((ref) {
  final Dio dio = Dio(BaseOptions(
    connectTimeout: const Duration(milliseconds: Constants.dioDefaultConnectTimeOut),
    receiveTimeout: const Duration(milliseconds: Constants.dioDefaultReceiveTimeOut),
  ));
  ...
  dio.interceptors.add(QueuedInterceptorsWrapper(
    onRequest: (options, handler) {
      ...
      if (options.extra.containsKey(Constants.customTimeOut)) {
        options.receiveTimeout = Duration(milliseconds: options.extra[Constants.customTimeOut]);
      }
      _logRequest(options.baseUrl, options.path, options.headers.toString(), options.data.toString());
      return handler.next(options);
    },
    onResponse: (response, handler) {
      _logResponse(response.requestOptions.baseUrl, response.requestOptions.path, response.headers.toString(), response.data.toString());
      return handler.next(response);
    },
    onError: (error, handler) {
      _logError(error.requestOptions.baseUrl, error.requestOptions.path, error.error.toString(), error.response?.data.toString());
